PseudoAbsGen<-function(input.file,outfile,method="KDE",bw.otim="adhoc",isopleth=95,bias=FALSE,template){
  ...
  if(bw.otim == "adhoc" | method == "MCP"){    
    ...
    if(method == "KDE"){
      ud <- kernelUD(xy, extent=.6, grid=150)
  ...
  if(bias){
    ...
    RasterInfo@file@datanotation <- "INT2U"
    NAval <- 0 
  }
  ...
  #need to explicitly define NA in writeStart call to maintain NA value integrity through write process [PSE]
  predrast <- writeStart(predrast, filename = filename, overwrite = TRUE, datatype = RasterInfo@file@datanotation, NAflag = NAval)
  ############################################################
  for (i in 1:tr$n) {
    if (i==tr$n) { 
