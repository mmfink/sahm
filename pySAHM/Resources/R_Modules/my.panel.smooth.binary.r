my.panel.smooth<-function (x, y, col = par("col"), bg = NA, pch = par("pch"),famly="binomial",
                           ...
                           weights=rep(1,times=length(y)),cex.mult,Ylab,plot.it=TRUE,lin=1,...)
{
  ...
  if(plot.it){
    ...
    else points(x, y, pch = pch,bg=c("blue","red")[factor(y,levels=c(0,1))],col=c("blue4","red4")[factor(y,levels=c(0,1))],cex=cex.mult,...)
